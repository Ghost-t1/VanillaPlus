...
public class ClearStatsCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender Sender, Command Command, String Label, String[] Args) {
        ...
        if (LastConfirmTime != null && (CurrentTime - LastConfirmTime) < CONFIRMATION_TIMEOUT) {
            ClearAllStats(Item);
            ConfirmationMap.remove(PlayerEntity.getUniqueId());
            PlayerEntity.sendMessage("§a✓ Вся статистика предмета очищена!");
            return true;
        }

        ConfirmationMap.put(PlayerEntity.getUniqueId(), CurrentTime);
        PlayerEntity.sendMessage("§e⚠ Повторите команду в течение 10 секунд для подтверждения");
